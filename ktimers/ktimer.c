...
void callback_timer(struct timer_list *data)
{
    printk(KERN_ALERT"timer callback function that is called is timer[%d]\n",count++);
    mod_timer(&Ktimer,jiffies + msecs_to_jiffies(Timeout));
}
...
static int chdev_init(void)
{
    ...
    timer_setup(&Ktimer,callback_timer,0);
